...
class ColaboradoresAdapter {
  ...
  async getColaboradores(params: {
    ...
    try {
      ...
      if (params.statusFilter && params.statusFilter !== 'todos') {
        colaboradoresFiltrados = colaboradoresFiltrados.filter(colaborador =>
          colaborador.status === params.statusFilter
        );
      }
      
      if (params.contratadaFilter && params.contratadaFilter !== 'todas') {
        colaboradoresFiltrados = colaboradoresFiltrados.filter(colaborador =>
          colaborador.contratadaId === params.contratadaFilter
        );
      }
   ...
   */
  async createColaborador(data: CreateColaboradorForm): Promise<ColaboradorDTO> {
    try {
   ...
   */
  async updateColaborador(id: string, data: UpdateColaboradorForm): Promise<ColaboradorDTO> {
    try {
   ...
   */
  async toggleStatusColaborador(id: string, novoStatus: 'ativo' | 'inativo'): Promise<void> {
    try {
  ...
  async getColaboradoresByContratada(contratadaId: string): Promise<ColaboradorDTO[]> {
    try {
      ...
      const result = await this.getColaboradores({ contratadaFilter: contratadaId });
      
      console.log('✅ Colaboradores da contratada carregados:', result.colaboradores.length);
