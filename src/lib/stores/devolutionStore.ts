...
import type { MovimentacaoEstoque, Entrega, FichaEPI } from '$lib/services/api/types';
import { 
  devolutionAdapter, 
...
export const validationCache = writable<Map<string, DevolutionValidation>>(new Map());
...
export const pendingDevolutions = derived(devolutionProcesses, $processes => 
  $processes.filter(p => p.status === StatusDevolucao.SOLICITADA || p.status === StatusDevolucao.EM_ANALISE)
);
...
export const devolutionsByStatus = derived(devolutionProcesses, $processes => {
  const byStatus = new Map<StatusDevolucao, DevolutionProcess[]>();
  
  Object.values(StatusDevolucao).forEach(status => {
    byStatus.set(status, $processes.filter(p => p.status === status));
  });
 ...
 */
export async function validateDevolutionRequest(entregaId: string): Promise<DevolutionValidation> {
  try {
 ...
 */
export async function requestDevolution(request: DevolutionRequest): Promise<string> {
  try {
    ...
    const newProcess: DevolutionProcess = {
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      estimatedCompletion: new Date(Date.now() + validation.estimatedProcessingTime * 60 * 60 * 1000)
    };
 ...
 */
export async function approveDevolution(processId: string, approverRole: string): Promise<void> {
  try {
 ...
 */
export async function finalizeDevolution(processId: string, condition: CondicaoEquipamento): Promise<void> {
  try {
 ...
 */
export async function loadDevolutionsByStatus(status: StatusDevolucao): Promise<void> {
  try {
    console.log('📋 Carregando devoluções com status:', status);

    const movimentacoes = await devolutionAdapter.listDevolutionsByStatus(status);
 ...
 */
export async function loadCollaboratorDevolutions(colaboradorId: string): Promise<DevolutionProcess[]> {
  try {
    console.log('👤 Carregando devoluções do colaborador:', colaboradorId);

    const movimentacoes = await devolutionAdapter.getDevolutionsByCollaborator(colaboradorId);
...
function updateDevolutionStats(): void {
  ...
  const stats: DevolutionStats = {
    ...
    solicitadas: processes.filter(p => p.status === StatusDevolucao.SOLICITADA).length,
    emAnalise: processes.filter(p => p.status === StatusDevolucao.EM_ANALISE).length,
    aprovadas: processes.filter(p => p.status === StatusDevolucao.APROVADA).length,
    finalizadas: processes.filter(p => p.status === StatusDevolucao.FINALIZADA).length,
    rejeitadas: processes.filter(p => p.status === StatusDevolucao.REJEITADA).length,
 ...
 */
function calculateAverageProcessingTime(processes: DevolutionProcess[]): number {
  const completed = processes.filter(p => p.status === StatusDevolucao.FINALIZADA);
